...
class OctiAlphaBetaZobristMoveOrderingPlayer(OctiPlayer):
    ...
    def alpha_beta_zobrist(self, zobrist_board: BoardStateWithZobristHash, depth: int, player_id: TokenType, alpha: float, beta: float, old_board: BoardState = None,
        ...
        for action in legal_actions:
            # # action_zobrist_key = self.calculate_zobrist_key(action.board, zobrist_board.board, zobrist_key, action.spaces_to_unxor, action.spaces_to_xor)
            # action_score = 0
            # if action_zobrist_key in self.transposition_table:
            #     # if depth - 1 in self.transposition_table[action_zobrist_key]:
            #     #     action_score = self.transposition_table[action_zobrist_key][depth - 1]
            #     else:
        ...
        if player_id == self.player_id:
            # maximizing player
            legal_actions = legal_actions[np.argsort(action_scores)[::-1]]  # Descending order
        else:
            # minimizing player
            legal_actions = legal_actions[np.argsort(action_scores)]  # Ascending order

        # legal_actions = sorted(legal_actions, key=lambda action: self.evaluate_board(action.board, self.player_id), reverse=True if player_id == self.player_id else False)
